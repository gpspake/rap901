import {AppearanceReleasePublic, ReleaseOut} from "../client"
import { Link} from "@tanstack/react-router"
import {ReleaseArtistsLink} from "./ReleaseArtistsLink.tsx";

interface ReleasesGridProps {
  releases: ReleaseOut[]|AppearanceReleasePublic[]
}

export const ArtistReleasesGrid = (props: ReleasesGridProps) => {
              ...
              {releases.map((release) => {
                  ...
                  return (
                    <li key={release.id}>
                      ...
                      {(release as ReleaseOut).artists && (
                        <p className="text-gray-50 text-xs">{
                          <ReleaseArtistsLink
                            className={'mt-2 block truncate'}
                            releaseArtists={(release as ReleaseOut).artists || []}
                          />}
                        </p>
                      )}
