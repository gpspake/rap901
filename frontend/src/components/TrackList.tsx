...
const getTrackArtists = (
  ...
  return trackArtists.reduce(
    (acc: TrackArtists, obj: TrackArtistPublic) => {
  ...
  return (
    <>
      {Object.keys(trackArtists.extra_artists).map((group) => (
        <span key={group}>
          {group} -{" "}
          <JoinedArtistLinks trackArtists={trackArtists.extra_artists[group]} />
          <br />
        </span>
      ))}
    </>
...
export const TrackList = (props: TrackListProps) => {
  ...
  const sortedTracks = tracks.sort(
    (a, b): number => Number(a.position) - Number(b.position),
  )
      ...
      <dl className="divide-y divide-white/10">
        {sortedTracks.map((track) => {
          ...
          return (
            <div
              key={track.id}
              className="px-4 py-1 grid grid-cols-8 sm:gap-8 sm:px-0"
            >
              <dt className="text-sm font-medium leading-6 text-white">
                {track.position}
              </dt>
              <dt className="text-sm font-medium leading-6 col-span-6 text-white">
                <JoinedArtistLinks trackArtists={trackArtists.artists} />{" "}
                {track.title} <br />
