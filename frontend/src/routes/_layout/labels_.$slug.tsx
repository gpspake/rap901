...
function getLabelQueryOptions({ slug }: { slug: string }) {
  return {
    queryFn: () => LabelsService.readLabel({ slug }),
                ...
                <div className="mx-auto max-w-2xl lg:max-w-5xl">
                  <div className="grid grid-cols-1 gap-y-16 lg:grid-cols-2 lg:grid-rows-[auto_1fr] lg:gap-y-12">
                    <h1 className="text-4xl tracking-tight sm:text-4xl text-zinc-100">
                      <span className="font-bold ">Label</span>  <span className="pl-2 text-slate-300 font-light">{label.name}</span>
                    </h1>
                  </div>
                  ...
                  {!!label.credits.length && (
                    <>
                      ...
                      <ReleasesGrid
                        releases={label.credits.sort((a, b) => a.year - b.year)}
                        columns={4}
                      />
                    </>
                  )}
