...
import { type ReleaseLabelLink, ReleasesService } from "../../client"
import { ProgressiveImg } from "../../components/ProgressiveImg.tsx"
import { ReleaseArtistsLink } from "../../components/Releases/ReleaseArtistsLink.tsx"
...
function getReleaseQueryOptions({ slug }: { slug: string }) {
  return {
    queryFn: () => ReleasesService.readRelease({ slug: slug }),
              ...
              <div className="mx-auto max-w-2xl lg:max-w-5xl">
                <div className="grid grid-cols-1 gap-y-16 lg:grid-cols-2 lg:grid-rows-[auto_1fr] lg:gap-y-12">
                  ...
                  <div className="lg:pl-20">
                    <div className="max-w-sm lg:max-w-none z-20 relative lg:inline-block mx-auto">
                      {frontImage && (
                        <ProgressiveImg
                          alt=""
                          loading="lazy"
                          ...
                          decoding="async"
                          data-nimg="1"
                          className="ml-3 -rotate-3 rounded bg-zinc-800 object-cover shadow-lg shadow-black"
                    ...
                    <div className="max-w-sm lg:max-w-none z-10 mx-auto">
                      {backImage && (
                        <ProgressiveImg
                          alt=""
                          loading="lazy"
                          ...
                          decoding="async"
                          data-nimg="1"
                          className="-ml-2 md:block rotate-3 rounded object-cover bg-zinc-800 shadow-lg shadow-black"
                          style={{color: "transparent"}}
                          sizes="(min-width: 1024px) 32rem, 20rem"
                  ...
                  <div className="lg:order-first lg:row-span-2">
                    ...
                    <div className="flex items-center py-4">
                      {labels
                        ?.filter(
                          (labelLink) =>
                            labelLink.entity_type?.name === "Label",
                        )
                        .map((label_link) => (
                          <div
                    ...
                    {release.discogs_url && release.discogs_url !== "Sealed" && (
                      <a href={release.discogs_url} className="font-medium text-sky-600 hover:underline" >
                        View on Discogs
                      </a>
                    )}
                    ...
                    <div className="mt-6 space-y-7 text-base text-zinc-400">
                      <h2 className="font-bold tracking-tight text-xl text-zinc-100">
                        Tracklist
                      </h2>
                      {release.tracks && (
                        ...
                        <ul>
                          {companies.map((releaseLabel) => (
                            <li className="text-gray-300" key={releaseLabel.id}>
                              {!!releaseLabel?.entity_type?.name &&
                                `${releaseLabel.entity_type.name} - `}
                              <Link
                                to="/labels/$slug"
                                className="hover:underline text-red-600"
                                params={{
                                  slug: releaseLabel.label.slug || "",
                                }}
                              >
                      ...
                      <>
                        ...
                        <ul>
                          {release.artist_links
                            .filter(
                              (releaseArtistLink) =>
                                  !!releaseArtistLink?.role?.name,
                            )
                            .map((release_artist_link) => (
                              <li className="text-gray-300" key={release_artist_link.id}>
                                {!!release_artist_link.role &&
                                  `${release_artist_link.role.name} - `}
                                <Link
                                  to="/artists/$slug"
                                  className="hover:underline text-red-600"
                                  params={{
                                    slug: release_artist_link.artist.slug,
                                  }}
                                >
                                  {release_artist_link.artist.name}
                                </Link>
                              </li>
                            ))}
                        </ul>
